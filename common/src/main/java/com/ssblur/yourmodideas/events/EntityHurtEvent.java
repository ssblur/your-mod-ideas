...
public class EntityHurtEvent implements EntityEvent.LivingHurt {
  ...
  public EventResult hurt(LivingEntity entity, DamageSource source, float amount) {
    ...
    if(YourModIdeasGameRules.getValue(level, YourModIdeasGameRules.EMOTIONAL_DAMAGE)) {
      if (entity instanceof Player player) {
        ...
        if(I18n.exists("message.yourmodideas.emotional_damage_%s_0".formatted(source.getMsgId()))) {
          int total = 0;
          while(I18n.exists("message.yourmodideas.emotional_damage_%s_%d".formatted(source.getMsgId(), total)))
            total++;
